...
const FeesBalanceReport = () => {
  ...
  try {
    ...
    const studentResponse = await axios.get('https://school-management-server-1pvb.onrender.com/api/students/search', {
      params: {
    ...
    if (studentResponse.data.success && studentResponse.data.student) {
      ...
      const paymentResponse = await axios.get('https://school-management-server-1pvb.onrender.com/api/fees', {
        params: {
  ...
  const handleFetchDefaulters = async () => {
    setLoading(true);
    try {
      const response = await axios.get("https://school-management-server-1pvb.onrender.com/api/fees/defaulters");
